...
public class buildCity : MonoBehaviour
{
    ...
    {	
    	...
    	seed = 38;
    	Debug.Log(seed);
    	//float seed = 42; (if want same city generated everytime)
    	//going through loop and generating a value of PerlinNoise for each point of the width and height.  
        for (int h = 0; h < mapHeight; h++)
        {
        	for (int w = 0; w < mapWidth; w++)
        	{
        		int result = (int) (Mathf.PerlinNoise(w/10.0f + seed,h/10.0f + seed) * 10); //gives Perline noise width and height of our map
